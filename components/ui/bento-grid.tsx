...
export const BentoGrid = ({ className, children }: { className?: string; children?: React.ReactNode }) => (
  <div className={cn('mx-auto grid max-w-7xl grid-cols-1 gap-4 sm:grid-cols-2 md:auto-rows-[18rem] md:grid-cols-3 ', className)}>
    {children}
  </div>
);
  ...
  return (
    ...
    >
      <div className={`${id === 6 && 'flex justify-center'} h-full`}>
        <div className={`absolute h-full w-full ${id === 1 || id === 4 ? 'bg-black/50' : ''}`}>
          {img && <img src={img} alt={img} className={cn(imgClassName)} />}
        </div>
        <div className={`absolute -bottom-5 right-0 ${id === 5 && 'w-full opacity-80'}`}>
          {spareImg && <img src={spareImg} alt={spareImg} className="h-full w-full object-cover" />}
        </div>
        <div className={`${id === 1 ? 'h-full' : ''}`}>
          ...
          >
            <div
              className={`z-10 font-sans text-sm font-extralight ${id === 1 ? 'text-white' : 'text-[#c1c2d3]'} md:text-xs md:font-normal lg:text-base`}
            >
              {description}
            </div>
            <div className="z-50 max-w-96 font-sans text-lg font-bold lg:text-3xl ">{title}</div>
